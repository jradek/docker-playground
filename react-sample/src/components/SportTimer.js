  ...
  return (
    <div className="card-panel">
      ...
      <div className="progress big">
        <div className="determinate" style={{ width: `${props.valuePercent}%` }}></div>
      </div>
    </div>
  );
}
...
class SportTimer extends Component {
  ...
  timerFunc = () => {
    ...
    const percent = Math.round((now - this.state.wallclockStartMs) / (this.state.wallclockDeadlineMs - this.state.wallclockStartMs) * 100);
    console.log("update raw = ", raw, ", rounded = ", rounded, ", percent ", percent)
      ...
      <div>
        ...
        <div className="row">
          <div className="input-field col s6">
            <input placeholder="seconds" id="interval" type="number" value={this.state.interval} min="0"
              onChange={e => { this.setState({ interval: e.target.value }) }}
            />
            <label htmlFor="interval">Interval in seconds</label>
          </div>
          <div className="input-field col s3">
            <button className="btn waves-effect" style={{ width: '100%' }} onClick={this.handleStart}>Start</button>
          </div>
          <div className="input-field col s3">
            <button className="btn waves-effect" style={{ width: '100%' }} onClick={this.handleStop}>Stop</button>
          </div>
        </div>
        <SimpleDisplay value={this.state.value} valuePercent={this.state.valuePercent} />
      </div>
    )
  }
}
